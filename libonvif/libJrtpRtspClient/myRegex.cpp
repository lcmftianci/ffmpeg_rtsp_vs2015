...
#ifndef _WIN32
MyRegex::MyRegex():	pReg(NULL), pMatch(NULL), NextPos(string::npos), LastStr(NULL)
#else
MyRegex::MyRegex() : pReg(NULL)/*, pMatch(NULL)*/, NextPos(string::npos), LastStr(NULL)
#endif
{
...
int MyRegex::Regex(const char * str, const char * pattern, list<string> * groups, bool ignore_case)
{
	...
	pReg = NULL; pMatch = NULL;
#else
	pReg = (regex*)calloc(1, sizeof(regex));
	//int RegFlags = std::REG_EXTENDED;
	std::regex express(pattern);
	std::match_results<std::string::iterator> results1;
	string strText = str;
	if (std::regex_search(strText.begin(), strText.end(), results1, express))
	{
